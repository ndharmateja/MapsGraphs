...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		for (int i = 0; i < this.getNumVertices(); i++) {
			sequence.add(this.getNeighbors(i).size() + this.getInNeighbors(i).size());
		}
	...
	public static void main (String[] args) {
		...
		Graph graph = new GraphAdjMatrix();
		//GraphLoader.loadRoadMap("data/maps/hollywood_large.map", graph);
		GraphLoader.loadRoadMap("data/testdata/simpletest.map", graph);
		System.out.println(graph);
		for (int i = 0; i < graph.getNumVertices(); i += 1){
			//System.out.println(i + "\t" + graph.getNeighbors(i));
			System.out.println(i + "\t" + graph.getDistance2(i));
		}
		//for (int i = 0; i < graph.getNumVertices(); i += 1){
		//	System.out.println(i + "\t" + graph.getInNeighbors(i));
		//}
